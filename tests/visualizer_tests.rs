...
mod visualizer_tests {
    ...
    fn test_path_entry_creation() {
        ...
        let entry = create_entry(0, "C:\\Windows", &paths);
    ...
    fn test_path_with_spaces_unquoted() {
        ...
        let entry = create_entry(0, "C:\\Program Files", &paths);
        assert!(entry.has_spaces);
        assert!(!entry.is_quoted);
    ...
    fn test_user_specific_path_detection() {
        ...
        let entry = create_entry(0, "C:\\Users\\test\\.cargo\\bin", &paths);
    ...
    fn test_user_specific_appdata() {
        ...
        let entry = create_entry(0, "C:\\Users\\test\\AppData\\Local", &paths);
        assert!(entry.is_user_specific());
    }
    ...
    fn test_non_existent_path() {
        ...
        let entry = create_entry(0, "C:\\NonExistent123456789", &paths);
        assert!(!entry.exists);
        assert!(entry.has_issues());
    }
    ...
    fn test_warnings_for_non_existent() {
        ...
        assert!(warnings
            .iter()
            .any(|w: &String| w.contains("does not exist")));
    }
    ...
    fn test_warnings_for_unquoted_spaces() {
        ...
        let entry = create_entry(0, "C:\\Program Files", &paths);
        let warnings = entry.get_warnings();
        assert!(warnings.iter().any(|w: &String| w.contains("not quoted")));
    }
    ...
    fn test_warnings_for_duplicate() {
        ...
        let entry = create_entry(0, "C:\\Windows", &paths);
        let warnings = entry.get_warnings();
        assert!(warnings.iter().any(|w: &String| w.contains("Duplicate")));
    }
    ...
    fn test_warnings_for_long_path() {
        ...
        let entry = create_entry(0, &long_path, &paths);
        let warnings = entry.get_warnings();
        assert!(warnings
            .iter()
            .any(|w: &String| w.contains("260 characters")));
    }

    #[test]
    fn test_no_warnings_for_clean_path() {
        let paths = vec![spath_cli::constants::WINDOWS_PATH.to_string()];
        let entry = create_entry(0, spath_cli::constants::WINDOWS_PATH, &paths);
        let warnings = entry.get_warnings();
        assert!(warnings.is_empty());
    }
    ...
    fn test_program_files_not_user_specific() {
        ...
        let entry = create_entry(0, spath_cli::constants::PROGRAM_FILES, &paths);
        assert!(!entry.is_user_specific());
    }
    ...
    fn test_dotnet_path_is_user_specific() {
        ...
        let entry = create_entry(0, "C:\\Users\\test\\.dotnet", &paths);
        assert!(entry.is_user_specific());
    }
    ...
    fn test_npm_path_is_user_specific() {
        ...
        let entry = create_entry(0, "C:\\Users\\test\\.npm", &paths);
        assert!(entry.is_user_specific());
    }
